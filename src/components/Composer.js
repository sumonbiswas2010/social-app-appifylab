...
export default function Composer({ me, onCreated }) {
  ...
  async function submit() {
    if (posting || (!content.trim() && !image)) return;
    setPosting(true);
    setError('');
    try {
      ...
      const post = await apiCall('/api/posts', { method: 'POST', body: form });
      setContent('');
      removeImage();
      setPrivacy('public');
      onCreated(post);
  ...
  const disabled = posting || (!content.trim() && !image);

  return (
    <div className="_feed_inner_text_area _b_radious6 _padd_b24 _padd_t24 _padd_r24 _padd_l24 _mar_b16">
      <div className="_feed_inner_text_area_box">
        <div className="_feed_inner_text_area_box_image">
          <Avatar user={me} size="h-10 w-10" />
        </div>
        <div className="form-floating _feed_inner_text_area_box_form">
          ...
          ></textarea>
          {!content && (
            <label className="_feed_textarea_label flex! gap-2" htmlFor="floatingTextarea">
              Write something ...
      ...
      {preview && (
        <div style={{ position: 'relative', margin: '12px 0 0' }}>
          <img src={preview} alt="preview" style={{ maxHeight: 320, maxWidth: '100%', borderRadius: 6 }} />
          <button
            type="button"
            onClick={removeImage}
      ...
      {error && <p style={{ color: '#e0245e', fontSize: 14, margin: '10px 0 0' }}>{error}</p>}
      ...
      <div className="_feed_inner_text_area_bottom">
        ...
        <div className="_feed_inner_text_area_btn" style={{ display: 'flex', alignItems: 'center' }}>
          <select value={privacy} onChange={(e) => setPrivacy(e.target.value)} aria-label="Post privacy" style={selectStyle}>
            <option value="public">Public</option>
            <option value="private">Private</option>
          </select>
          <button type="button" className="_feed_inner_text_area_btn_link" onClick={submit} disabled={disabled} style={disabled ? { opacity: 0.6 } : undefined}>
            {posting ? <Spinner /> : <SendIcon />} <span>{posting ? 'Posting...' : 'Post'}</span>
          </button>
        </div>
      </div>
      ...
      <div className="_feed_inner_text_area_bottom_mobile">
        <div className="_feed_inner_text_mobile">
          <div className="_feed_inner_text_area_item">
            <div className="_feed_inner_text_area_bottom_photo _feed_common">
              <button type="button" className="_feed_inner_text_area_bottom_photo_link" onClick={() => fileRef.current?.click()} aria-label="Add photo">
                <span className="_feed_inner_text_area_bottom_photo_iamge _mar_img">
                  <PhotoIcon />
                </span>
              </button>
            </div>
            <select value={privacy} onChange={(e) => setPrivacy(e.target.value)} aria-label="Post privacy" style={{ ...selectStyle, marginRight: 0 }}>
              <option value="public">Public</option>
              <option value="private">Private</option>
            </select>
          </div>
